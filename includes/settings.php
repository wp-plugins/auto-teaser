		...
		<tr id="row-teaser-format">
			...
			<td>
				...
				<p>
					...
					<a href="#" id="hide-teaser-format-preview" hidden="hidden"><?php _e("Hide preview", "auto_teaser"); ?></a>
					&vert;
					<a href="#" id="show-placeholders"><?php _e("Show placeholders", "auto_teaser"); ?></a>
								...
								if(count($random_post->post_category)) {
									...
									if(count($random_post->post_category) > 1) {
										foreach($random_post->post_category as $category_id) {
											array_push($placeholder_examples["categories"], get_cat_name($category_id));
										}
									}
								}
